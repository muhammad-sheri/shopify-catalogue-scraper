...
export function Hero() {
  return (
    ...
    >
      {/* Two soft lights rather than a flat fill, so the slab has depth without
          needing an image the page would have to load. The cool one carries the
          accent hue; the warm one keeps the black from going blue-grey. */}
      <div
        aria-hidden
        className="pointer-events-none absolute -right-24 -top-32 size-[26rem] rounded-full opacity-70 blur-3xl"
        style={{ background: "radial-gradient(circle, var(--hero-glow-a), transparent 65%)" }}
      />
      <div
        aria-hidden
        className="pointer-events-none absolute -left-20 top-10 size-80 rounded-full opacity-80 blur-3xl"
        style={{ background: "radial-gradient(circle, var(--hero-glow-b), transparent 65%)" }}
      />
      ...
      <div className="relative mx-auto flex max-w-6xl flex-col gap-6">
        <div className="flex items-center justify-between gap-4">
          ...
          >
            <span className="size-1.5 rounded-full" style={{ background: "var(--hero-dot)" }} />
